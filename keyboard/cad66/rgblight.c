...
#define rgblight_timer_init() do { DDRF |= (1<<PF0);} while(0)
#define rgblight_timer_enable() do { PORTF &= ~(1<<PF0); PORTB |= (1<<PB0);} while(0)
#define rgblight_timer_disable() do { PORTF |= (1<<PF0); if (!(backlight_config.enable)) PORTB &= ~(1<<PB0);} while(0)
#define rgblight_timer_enabled !(PORTF&(1<<PF0))
...
typedef struct
{
    ...
    uint8_t g_pwm_reg;
    uint8_t b_pwm_reg;
} pwm_rgb_control_register;

const pwm_rgb_control_register g_pwm_rgb_control_register[] PROGMEM = {
    { 0x33, 0x43, 0x53 },
    { 0x32, 0x42, 0x52 },
    { 0x31, 0x41, 0x51 },
    { 0x30, 0x40, 0x50 },
...
void rgblight_set(void)
{
    ...
    for (uint8_t i=0; i<RGBLED_NUM; i++) {
        g_pwm_buffer[ pgm_read_byte(&g_pwm_rgb_control_register[i].r_pwm_reg)-0x24 ] = led[i].r;
        g_pwm_buffer[ pgm_read_byte(&g_pwm_rgb_control_register[i].g_pwm_reg)-0x24 ] = led[i].g;
        g_pwm_buffer[ pgm_read_byte(&g_pwm_rgb_control_register[i].b_pwm_reg)-0x24 ] = led[i].b;
    }
